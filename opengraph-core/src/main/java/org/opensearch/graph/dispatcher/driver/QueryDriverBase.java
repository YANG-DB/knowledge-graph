...
import org.opensearch.graph.dispatcher.utils.GraphApiUtils;
...
import org.opensearch.graph.model.query.ParameterizedQuery;
...
import org.opensearch.graph.model.query.properties.constraint.ParameterizedConstraint;
import org.opensearch.graph.model.query.properties.constraint.QueryNamedParameter;
import org.opensearch.graph.model.resourceInfo.*;
import org.opensearch.graph.model.results.AssignmentUtils;
...
import static org.opensearch.graph.model.Utils.getOrCreateId;
import static org.opensearch.graph.model.asgQuery.AsgCompositeQuery.hasInnerQuery;
import static org.opensearch.graph.model.transport.CreateQueryRequestMetadata.QueryType.parameterized;
    ...
    public Optional<Object> run(Query query, int pageSize, String cursorType) {
        ...
        try {
            CreateQueryRequest queryRequest = new CreateQueryRequest(id, id, query,
                    CursorFactory.request(query.getOnt(),cursorType,new CreatePageRequest(pageSize)));
    ...
    public Optional<Object> runCypher(String cypher, String ontology, int pageSize, String cursorType) {
        ...
        try {
            CreateJsonQueryRequest queryRequest = new CreateJsonQueryRequest(id, id, TYPE_CYPHERQL, cypher, ontology,
                    CursorFactory.request(ontology,cursorType,new CreatePageRequest(pageSize)));
            Optional<QueryResourceInfo> resourceInfo = create(queryRequest);
            if (!resourceInfo.isPresent())
                return Optional.empty();
