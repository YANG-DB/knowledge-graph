package org.opensearch.graph.unipop.schema.providers.logging;
    ...
    public Optional<GraphElementPropertySchema> getPropertySchema(String name, String contextEntity) {
        ...
        try {
            new LogMessage.Impl(this.verboseLogger, trace, "start getPropertySchema name: {}", sequence, LogType.of(start), getPropertySchema, ElapsedFrom.now())
                    .with(name).log();
            Optional<GraphElementPropertySchema> propertySchema = this.schemaProvider.getPropertySchema(name,contextEntity );
