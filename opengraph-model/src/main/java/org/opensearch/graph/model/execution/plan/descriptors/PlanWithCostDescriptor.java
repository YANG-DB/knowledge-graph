...
/*-
 *
 * PlanWithCostDescriptor.java - opengraph-model - yangdb - 2,016
...
import org.opensearch.graph.model.execution.plan.composite.*;
...
import org.opensearch.graph.model.execution.plan.entity.*;
...
import static org.opensearch.graph.model.execution.plan.composite.descriptors.IterablePlanOpDescriptor.getIterablePlanOpDescriptor;
...
public class PlanWithCostDescriptor<P, C> implements Descriptor<PlanWithCost<P, C>>, GraphDescriptor<PlanWithCost<Plan, PlanDetailedCost>> {
    ...
    public void dot(PlanWithCost<Plan, PlanDetailedCost> plan, StringBuilder root) {
        ...
        while (iterator.hasNext()) {
            ...
            if (GoToEntityOp.class.isAssignableFrom(next.getClass())) {
                QueryDescriptor.removeRedundentArrow(root);
                root.append("\n");
    ...
    private String print(AsgEBasePlanOp planOp) {
        return QueryDescriptor.printElements(Collections.singletonList(planOp.getAsgEbase().geteBase()));
    }
    ...
    public static String printProps(BasePropGroup element) {
        ...
        prpoBuilder.append("\n " + element.geteNum() + "->" + QueryDescriptor.printElements(props));
        QueryDescriptor.removeRedundentArrow(prpoBuilder);
