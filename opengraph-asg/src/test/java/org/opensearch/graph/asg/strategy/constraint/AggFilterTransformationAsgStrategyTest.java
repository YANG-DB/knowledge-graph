...
public class AggFilterTransformationAsgStrategyTest{
    //region Setup
    @Before
    ...
    public void testAsgFilterAggTransformationStrategyVerifyAggTest() throws Exception {
        AsgQuery query = Q1();

        AsgStrategyContext context = new AsgStrategyContext(ont);
        AggFilterTransformationAsgStrategy asgStrategy = new AggFilterTransformationAsgStrategy();

        asgStrategy.apply(query,context);

        //Setting The EProp expression as a date represented by Long value

        Object expr = ((EProp) AsgQueryUtil.element(query, 4).get().geteBase()).getCon().getExpr();
