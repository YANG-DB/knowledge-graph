...
import org.opensearch.graph.test.etl.DateFieldPartitioner;
...
public abstract class ETLUtils {
    ...
    public static IndexPartitions indexPartition(String label) {
        ...
        Optional<IndexPartitions> partition = entity.stream().filter(p -> ((Map) p).containsKey(DragonsOntologyPhysicalIndexProviderFactory.PARTITION)).map(v -> {
            if (((Map) v).get(DragonsOntologyPhysicalIndexProviderFactory.PARTITION).equals(DragonsOntologyPhysicalIndexProviderFactory.TIME)) {
                return new DragonsOntologyPhysicalIndexProviderFactory.TimeBasedIndexPartitions((Map) v);
            } else {
    ...
    public static Iterable<String> indices(Map map) {
        JSONArray array = (JSONArray) ((Map) (map).get(DragonsOntologyPhysicalIndexProviderFactory.PROPS)).get(DragonsOntologyPhysicalIndexProviderFactory.VALUES);
