...
import static org.opensearch.graph.test.scenario.ETLUtils.getBulkProcessor;
import static org.opensearch.graph.test.scenario.ETLUtils.getClient;

/**
 * Created by Roman on 07/06/2017.
 */
public class IngestRegisteredInES {
    public static void main(String[] args) throws IOException, InterruptedException, ExecutionException {
        TransportClient client = getClient();
        IntStream.range(1,13).forEach(p -> {
            try {
                writeToIndex("C:\\demo_data_6June2017\\registered_chunks", "kingdomsRelations_REGISTERED_GUILD-out", "2000" +String.format("%02d", p), client);
            } catch (IOException e) {
                e.printStackTrace();
